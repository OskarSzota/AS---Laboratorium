...
elseif (isset($params['updatethumbsbutton']))
{
  ...
  if ($result && $result->RecordCount() > 0)
  {
    ...
    {
      Gallery_utils::DeleteFiles(Gallery_utils::DefaultGalleryThumbsPath(), $row['fileid'] . '-*', false);
      $filepath = $row['filepath'];
    }
    $filepath .= $filepath != '' ? '/' : '';
    Gallery_utils::DeleteFiles(str_replace('/', DIRECTORY_SEPARATOR, Gallery_utils::DefaultGalleryPath() . $filepath), IM_PREFIX . '*', false);
  }
  if ($result)
  {
...
elseif (isset($params['directoryname']))
{
  ...
  {
    if (!mkdir(Gallery_utils::DefaultGalleryPath() . $gallerypath . $params['directoryname']))
    {
