...
else
{
  ...
  foreach ($gallery as $file)
  {
    ...
    {
      $numberofimages++;
      $onerow->thumburl = Gallery_utils::DefaultGalleryUrl() . str_replace('%2F', '/', rawurlencode($file['filepath'] . IM_PREFIX . $file['filename']));
      $onerow->thumb = '<img src="' . $onerow->thumburl . '" alt="' . $file['filename'] . '" />';
      $onerow->filename = $file['filename'];
      ...
      $onerow->isdir = 0;
      if (!file_exists(Gallery_utils::DefaultGalleryPath() . $file['filepath'] . IM_PREFIX . $file['filename']))
      {
        Gallery_utils::CreateThumbnail(Gallery_utils::DefaultGalleryPath() . $file['filepath'] . IM_PREFIX . $file['filename'], Gallery_utils::DefaultGalleryPath() . $file['filepath'] . $file['filename'], IM_THUMBWIDTH, IM_THUMBHEIGHT, 'sc');
      }
...
$smarty->display($this->GetTemplateResource('editgallery.tpl'));
?>